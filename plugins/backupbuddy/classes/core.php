...
class backupbuddy_core {
	...
	public static function php_runtime_test( $schedule_results = false ) {
		...
		while( $loop_count <= $total_loops ) {
			...
			while( ( microtime( true ) - $loop_start ) < 1 ) { // Can't sleep() because sleep doesn't count towards execution time. Delays 1 sec of CPU time towards PHP counter.

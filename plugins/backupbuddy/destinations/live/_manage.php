...
if ( '' != pb_backupbuddy::_GET( 'live_action' ) ) {
	...
	/*} elseif ( 'view_stash_files' == $action ) {
		...
		*/
	...
	} elseif ( 'last_snapshot_status' == $action ) {
		if ( '' == $state['stats']['last_remote_snapshot_id'] ) {
			...
			echo '<h3>Last Snapshot Status:</h3>';
			...
			if ( ! is_array( $response ) ) {
				...
				echo '<textarea readonly="readonly" style="width: 100%;" wrap="off" cols="65" rows="20">' . print_r( $response, true ) . '</textarea>';
				echo '<br><br><br>';
			}
		}
...
<div class="backupbuddy-live-advanced-troubleshooting-wrap" style="display: none; background: #E5E5E5; padding: 15px; margin-top: 15px; border-radius: 10px;">
	...
	<a href="<?php echo pb_backupbuddy::nonce_url( $admin_url . '?page=pb_backupbuddy_live&live_action=last_snapshot_status' ); ?>" class="button button-secondary button-tertiary" style="<?php if ( '' == $state['stats']['last_remote_snapshot_id'] ) { echo 'opacity: 0.4;'; } ?>">Last Snapshot Status</a>
